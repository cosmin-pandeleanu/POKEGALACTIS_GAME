...
public class Level1State extends GameState {
    ...
    */
	public void init() {
		...
		/// Background
		bg = new Background("/Backgrounds/level1_bg.png");
		...
		try {
			...
			titleLVL = new Title(text.getSubimage(0, 0, 220, 75));
			titleLVL.setY(60);
		}
		catch(Exception e) {
			e.printStackTrace();
    ...
    */
	private void populateEnemies() {
		...
		for (Point point : points) {
			b = new Blastoise(tileMap);
			b.setPosition(point.x , point.y);
			enemies.add(b);
			b2 = new Bastiodon(tileMap);
			b2.setPosition(point.x - 75, point.y);
			enemies.add(b2);
			n = new Ninjask(tileMap);
			n.setPosition(point.x + 150, point.y);
			enemies.add(n);
		}

		// The final enemy
		finalEnemies = new ArrayList<>();
		Point point = new Point(3750,200);
		Abomasnow a;
		a = new Abomasnow(tileMap);
    ...
    */
	private void reset() {
		...
		eventStart = true;
		eventStart();
		titleLVL = new Title(text.getSubimage(0, 0, 200, 75));
		titleLVL.setY(60);
	}
	...
	private void eventDead() {
		...
		if(eventCount >= 120) {
			if(player.getLives() == 1) {
				...
				DataBase.insertDB("GAME LOST ","LEVEL 1", PlayerSave.getScore(), PlayerSave.getLives() - 1 , PlayerSave.getHealth(), PlayerSave.getTimeToString());
                bgMusic.stop();
